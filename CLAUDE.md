...
- **核心取捨**：同一份能量，餵塔還是餵生產線。**塔持續消耗能量**——這是全案的心臟，不可移除。
- **塔＝角色**：全自動開火，射程內即攻擊，玩家不下戰鬥指令。生產與物流用另一套「建築」。
- **時間流**：純即時**不可暫停**；波與波之間有 45 秒準備期（可 4× 快進）；戰鬥期不可加速也不可減速。
- **自動化模型**：**流量網路**（每 tick 解容量受限比例分配），**不是**實體物品搬運。
- **雙層結構**：塔防局（主菜）＋潮汐公司 tycoon（可獨立遊玩的第二層）。
  - tycoon → 塔防：**只給橫向內容**（角色、建築、藍圖槽、難度鑰匙、外觀），**絕不給數值**。
  - 兩層資源池完全分離；共享唯一進度條＝**公司等級**。
- **每日挑戰使用統一起始配置** → 抽卡與掛機時數不影響競技公平。
- **設計紅線**（硬性禁令）：
  - **R1 無永久損失／刪檔**——失敗只花時間。
  - **R2 無體力值／等待閘門**——想玩多久就玩多久。
