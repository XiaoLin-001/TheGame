...
const DEFS := {
	...
	# ── M3 第三批（B3.2c）。補滿 §5 內容矩陣的 M2 欄（12 隻）──────────────
	#
	# ★ **這一批也在規格階段否決了一隻**：原訂「候潮 —— 準備期等越久，這一波的
	#   敵人越硬」，動詞是時間（讓提前召喚變成真的取捨）。否決理由是 **R-1**：
	#   純即時不可暫停會滑向手速遊戲，而那條風險登記在案的**對策就是波間準備期**。
	#   給準備期加時間壓力＝把自己的減災措施拆掉。
	#   **兩批被否決的兩隻是同一種錯**：規則本身不錯，代價卻由一條鎖定設計付。
	"riftling": {
		# ★ 分裂：死亡時裂成 2 隻裂片。前九隻的死亡是終點，於是「防線放在路徑
		#   哪一段」純粹看擺得下——這一隻讓**在哪裡殺**變成一個問題。
		#   剋制它的是濺射（碎浪一發連裂片一起清）與把火力往路徑前段推。
		#   ★ 視覺：輪廓中間一道裂縫（背景色切過去，和稜鏡的裂法同一招）。
		"name": "裂潮", "hp": 120.0, "speed": 0.8, "armor": 0.0, "barrier": 0.0,
		"dmg": 9.0, "value": 30, "radius": 10.0,
		"split": 2, "split_into": "riftshard",
	},
	"riftshard": {
		# ★ 裂片**價值 0**——連 25% 全域擊殺回收都拿不到。不然分裂是一台印鈔機：
		#   一隻 30 礦砂的敵人死掉會生出兩張額外的收據。
		#   ★ **它自己沒有 `split`**：裂片再分裂是無限遞迴，而症狀是遊戲當場停住。
		#   半徑 4 是全場最小（苔群 5），跑得比母體快——它從防線中段長出來。
		"name": "裂片", "hp": 36.0, "speed": 1.3, "armor": 0.0, "barrier": 0.0,
		"dmg": 4.0, "value": 0, "radius": 4.0,
	},
	"shroud": {
		# ★ 遮蔽：3 格內的塔射程 −2。射程到此為止只被玩家自己的升級改過，
		#   從來沒有被場上的東西改過——「射程圈算一次就算完」是一個免費答案。
		#   剋制它的是**升過射程的那一級**（錨・稜鏡・霜礁・定潮）與長哨（射程 12）。
		#   這是第一條讓「升這一座」而不是「多蓋一座」成為答案的規則。
		# ⚠ **下限 2 格，不是 0**：射程 0 的塔不算交戰（`Combat.engaged()` 只看
		#   幾何）→ 不必付交戰耗能 → 這條規則會反過來替玩家省電。
		#   ★ 視覺：外圈一段一段的虛線弧（它遮的是「看得多遠」，標記就畫在外圈）。
		"name": "遮潮", "hp": 95.0, "speed": 1.1, "armor": 0.0, "barrier": 0.0,
		"dmg": 8.0, "value": 30, "radius": 9.0,
		"veil_range": 3.0, "veil_cut": 2.0,
	},
	"silt": {
		# ★ 淤積：**它壓著的那條導管本 tick cap 減半**。導管的 cap 是準備期算得完的
		#   一個常數，而「幹線加粗過就永遠夠寬」是一個免費答案。
		#   剋制它的是**兩條獨立幹線**（第 10 關參考解教過的手法，§7.9）與儲槽
		#   （本地緩衝撐過它走過去的那幾秒）。
		# ⚠ 減的是 **cap 不是流量**：cap 是導管的寬度（§7.2 全案最重要的常數），
		#   減流量只是「這一 tick 少送一點」，減 cap 是「這條線這一 tick 變窄了」
		#   ——後者才會讓那條線在畫面上真的變色（滿載）。
		#   ★ 視覺：輪廓下緣拖一片沉積（它壓著線，要有重量感）。
		"name": "濁潮", "hp": 140.0, "speed": 0.7, "armor": 0.0, "barrier": 0.0,
		"dmg": 6.0, "value": 34, "radius": 12.0,
		"silt_cap": 0.5,
	},
}
...
static func endless_pool(w: int) -> Array[String]:
	...
	for gate: Array in [
		...
		# ⚠ **裂片不在這張表上**（B3.2c）。它是分裂生出來的，不是排程排出來的
		#   ——把它放進池裡，玩家會看到一隻沒有母體的裂片憑空出現，
		#   而「這東西是從哪來的」正是這條規則唯一要講清楚的事。
		#   `endless_test` 有一條斷言釘住它不在池裡（而不是「每一隻都在池裡」）。
	]:
		if w >= int(gate[0]):
...
## 這一隻是**分裂生出來的**，不進任何出場表（B3.2c）。
##
## 判準是「有沒有別人指名要生它」而不是一份手寫清單——加第二隻會分裂的敵人時，
## 它的子代自動也不進池，沒有人要記得補一列（`data/` 就是這個形狀）。
static func spawned_by_split(type: String) -> bool:
	if type == "":
		return false
	for parent: Variant in DEFS:
		if String((DEFS[parent] as Dictionary).get("split_into", "")) == type:
			return true
	return false
