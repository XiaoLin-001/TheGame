...
const NO_MODS := {
	...
	"produce_mult": 1.0,
	# ★ 難度層的兩格（B2.6 的 `data/Difficulty.gd`；科技樹本身沒有節點動它們）。
	#   骨架同樣列在這裡：讀取端一律無條件乘，而**沒有難度層的局**
	#   （戰役、測試圖、每日挑戰）走的正是這個 1.0 預設。
	"enemy_hp_mult": 1.0,      # 敵人血量倍率（乘在 §7.5 的 hp 與無盡曲線之上）
	"enemy_damage_mult": 1.0,  # 敵人對建築的每秒傷害倍率
}
