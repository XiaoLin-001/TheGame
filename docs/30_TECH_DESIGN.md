...
| `TL_DEMO_TICKS=<ticks>` | `TL_PANEL=battle` 的示範佈局先推幾個 tick 再交給畫面（預設 **860** ＝ 60 秒準備期 ＋ 26 秒戰鬥，敵潮剛進射程）。**推的是 tick 不是階段**，所以 `TL_DEMO_TICKS=N` 與 `TL_SIM=N` 是同一個局面；給小一點就拍得到準備期（階段色調、提前召喚倍率要兩張圖才比得出來） |
