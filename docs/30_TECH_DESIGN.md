...
**每 tick 的解算步驟**（依資源種類各跑一次，**順序固定：礦砂 → 能量 → 合金**）：

> **順序是規格不是實作細節**（B1.1）：發電機的能量產出乘它自己的礦砂滿足率，熔爐的合金產出乘它「礦砂與能量兩個滿足率中較低的那一個」。倒過來解會讓上游缺料延遲一個 tick 才傳到下游，玩家讀不出因果。**熔爐取 min 而不是相乘**——50% 電 ＋ 50% 礦不該得到 25% 產出，那比畫面上任何一條線都糟。
...
**兩者都是純資料**：儲槽的 `charge` 存在 `SessionState`，由解算器的回傳值更新，解算器本身仍是純函式（`solve(nodes, edges, priorities, topo := {}) -> deltas`；`topo` 是呼叫端算好的拓樸快取，省略即自行建立，見 §5.3）。
...
**★ `reach` 是「對每條有向邊各自成立」的量**：一條實體管線在解算圖上是兩條方向相反的邊（GDD §7.2 導管無方向），若只按**節點**算一份，A 會看到「經 B 下去還有需求」，而那份需求其實要繞回 A 自己——資源就在兩點之間來回彈，`flow` 灌水、線寬說謊。B0.5 把導管改成雙向時當場現形（一條 cap 28 的幹線報出 29.26 的流量）。

**★ `reach` 的求法（B2.1e 換過一次，§5.3）**：在**生成森林上重根**——後序求子樹的可送達需求（`down`）、前序把根重掛到每個節點（`up`），兩趟掃完就得到全部有向邊的值，`O(V+E)`。B0.5–B2.1d 的舊法是對**每一次推送**重跑一趟 DFS（`O(推送次數 × (V+E))`），在一屏地圖上綽綽有餘，在 64×40 的大圖上是災難（219 節點 1.4 秒/tick）。

> **兩個必要的補丁，各自都有回歸斷言守著**：
> - **非樹邊給「一跳」的量**（只看對面那一格自己的需求，不再往下看）。給 0 的話，玩家「兩條線一起餵同一個大消費者」的佈局裡會有一條完全不載流——等於偷偷禁掉迴路（`flow_test._two_lines_feed_one_consumer()`）。
> - **節點吸收掉需求時，沿祖先鏈 `O(深度)` 扣掉**。舊法每次推送重算，所以 `demand_left` 永遠是活的；新法一個迭代只算一次，不補這一刀就會拿上一刻的快照去分配，把餘量往已經吃飽的節點推（`flow_test._satisfied_nodes_stop_attracting()`）。
>
> **森林是純拓樸的**（不看殘餘容量），所以三張資源網、三次迭代共用同一棵，`FlowNetwork.prepare()` 一個 tick 算一次。根依索引遞增取、出邊依邊索引遞增走 → 同一份輸入永遠長出同一棵森林（§2.4 浮點紀律）。
>
...
| **`TL_PICK=wire`** ★ | 與 `TL_CLICKTEST` 併用時，合照那一張改成**選一條導管**（挑本 tick 流量最大的那條），拍得到導管的檢視面板。同 `TL_FOCUS` 的理由：**檢視面板是純互動狀態，沒有鉤子就拍不到**，於是「導管面板做好了」只能用宣稱的。預設（不給）＝選一座塔，維持 B3.6 那張圖 |
...
### 5.1 M1 實測（B1.7，2026-08-01）

| 情境 | 模擬單 tick | 渲染 | 判定 |
|---|---|---|---|
| 戰役第 1 關（7 節點／6 導管／40 敵人） | **0.79 ms** | — | ✅ 目標內 |
| 戰役第 2–3 關（15／15／40） | **1.9 ms** | — | ✅ 目標內 |
| 戰役第 4 關（21／22／40） | **4.7 ms** | — | 🟡 超出目標 3ms，在上限 8ms 內 |
| 戰役第 5 關（25／25／40） | **6.2 ms** | — | 🟡 同上（**M1 最重的一關**） |
| **RG-8 壓力情境**（547／2045／200） | **30,200 ms** | 42.6 ms（23 FPS） | ❌ **不通過**，改排 B2.1（風險 R-17） |

**根因**：傳播的拜訪次數是 `O(迭代 × (V+E)²)` 級——`FlowNetwork._reach` 對**每一次推送**
重跑一次「下游還有多少需求」的 DFS，memo 不跨推送者重用（那個取捨在原始碼裡就寫著）。
節點數不是主因，**邊數才是**：同樣 17 個節點，密集網（42 條導管）比戰役關卡慢一個數量級。

**B1.7 做了什麼**：把每次拜訪都要重算的常數（邊的 `to`／`cap`、節點的優先權）提到迴圈外，
換成 `PackedInt32Array`／`PackedFloat64Array`。**不動任何算式、不動運算順序**，
所以結果逐位元相同（`campaign_test` 416 項與 `TL_SIM` 基準逐欄相同是這件事的證明）。
第 5 關 9.2 → 6.2 ms（−32%），壓力情境 55 → 30 秒（−46%）。

**B1.7 沒做什麼、為什麼**：要真正壓下 RG-8 得換掉 `_reach` 的估計方式，而**那會動到
分配比例** → 五關的難度校準（`10_GDD.md` §7.9）全部要重做。那是一個批次的工作量，
不是 M1 驗收該順手做的事。它的正確位置是 B2.1——程序生成大圖才是真的會撞到這條線的地方。

> ⚠ **`PackedFloat32Array` 不可以**：單精度會把 cap 降級，那就不是「一樣的計算跑得比較快」，
> 是換了一組數字。快取式最佳化的唯一價值就是「結果不變」，一旦變了就變成一次沒有驗收條件的改版。

### 5.2 無盡大圖實測（B2.1b，2026-08-02）——★ R-17 由「監控中」轉為「已觸發」

B1.7 把 R-17 的判定推遲到「大圖才是真的會撞到它的地方」。大圖做出來了，**它撞到了**。

佈局刻意不是 RG-8 的壓力情境（分支度 7.5，那是量測台不是玩法），而是**玩家真的會蓋出來的形狀**：64×40 圖上節點間隔 2 格、只往右與往下接（分支度約 4）。

| 規模 | 節點／導管 | 模擬單 tick | 判定 |
|---|---|---|---|
| 10×6 網（54／80／40 敵人） | 54／80 | **5.8 ms** | ✅ 在 8ms 上限內 |
| 18×11 網（180／298／40 敵人） | 180／298 | **22.5 ms** | ❌ **超上限 2.8 倍** |
| 31×19 網（565／1021／40 敵人） | 565／1021 | **92.5 ms** | ❌ 吃掉 92% 的 tick（`TICK = 100 ms`） |

**這條曲線的意義**：R-17 的門檻大約在 **100 個節點**。34×18 的一屏圖上很難蓋到那裡（第 5 關的參考解是 25 個），**64×40 的圖上輕易做得到**——這正是把地圖放大所買到的東西，也正是它的代價。

還跑得動（22.5 ms < 100 ms 的 tick），但 8ms 上限存在的理由是**留給渲染的餘裕**；超過它就開始和 60Hz 的畫面搶時間。565 節點那一格已經是「模擬吃掉整個 tick」。

→ **`40_PRODUCTION_PLAN.md` 的 B2.1d 從條件批轉為必做批。** 修法與代價不變（換掉 `_reach` 的估計方式 → 動到分配比例 → 五關校準全部重做）。

> ⚠ **§5.2 這張表的數字全部是錯的（低估約 50 倍），B2.1e 才發現。** 量測台的佈局
> 只放中繼／發電機／錨——**一顆採集器都沒有**。中繼與錨不產礦，發電機的能量產出
> 又要乘上自己的礦砂滿足率，於是全網供給恆為 0，`FlowNetwork` 的傳播迴圈
> **一次都沒有跑過**。那條曲線量到的是建索引的成本，不是解算的成本。
> 真值見 §5.3（219 節點：27 ms → **1,449 ms**）。表留著不刪，它是「量測台自己也要驗」
> 這條教訓的證據（`50_QA_PLAN.md` RG-128）。

### 5.3 R-17 修複實測（B2.1e，2026-08-03）

**先修量測台。** §5.2 的無盡曲線是一張死網（見上方警告），`perf_test._endless_scale()`
現在會把採集器蓋在礦點上，並補一條斷言 `供給 > 0`——**量測台自己也要有驗收條件**，
否則「效能沒問題」和「什麼都沒跑」在輸出上長得一模一樣。

| 情境 | 節點／導管 | B2.1e 前 | B2.1e 後 | 倍率 |
|---|---|---|---|---|
| 戰役第 1 關 | 7／6 | 0.79 ms | **0.46 ms** | 1.7× |
| 戰役第 4 關 | 21／22 | 4.70 ms | **0.88 ms** | 5.3× |
| 戰役第 5 關（M1 最重） | 25／25 | 6.13 ms | **1.03 ms** | 6.0× |
| 規模哨兵 | 65／210 | 508 ms | **3.4 ms** | 149× |
| 無盡 10×6（**live**） | 82／105 | 71.5 ms | **2.4 ms** | 30× |
| 無盡 18×11（**live**） | 219／373 | 1,449 ms | **7.8 ms** | 186× |
| 無盡 31×19（**live**） | 610／1107 | — | **20.7 ms** | — |
| **RG-8 壓力情境** | 547／2045／200 敵人 | 30,200 ms | **31.0 ms** | 974× |

**五關全部進入 3 ms 目標內**（B1.7 時第 4、5 關是黃燈）。

**四筆改動，前三筆逐位元相同、只有第一筆動到結果**：

1. **可送達需求改用生成森林上的重根法**（`FlowNetwork._reach_pass`）。舊法對**每一次推送**
   重跑一趟 DFS ＝ `O(推送次數 × (V+E))`；新法一個迭代兩趟掃完 ＝ `O(V+E)`。
   兩者算的都是同一個近似量——**有環圖上不存在精確解**，這正是 §2.5「已知近似」在講的事。
   ★ 非樹邊給**一跳**的量（只看對面那一格自己的需求）。給 0 的話玩家「兩條線一起餵同一個
   大消費者」的佈局會有一條完全不載流，等於偷偷禁掉迴路（`flow_test` 那一條當場紅）。
   ★ 節點吸收掉需求時，沿祖先鏈 `O(深度)` 把森林上看得到那份需求的量一起扣掉。
   **少了這一刀，淺灘示範佈局掉 55% 產出**（`delivered_total` 4412.8 → 2008.4）：
   採集器把礦推給發電機、發電機吃飽了，上游的中繼卻還拿著上一刻的快照，
   於是把三分之二的量原路推回去。回歸斷言在 `flow_test._satisfied_nodes_stop_attracting()`。
2. **`BattleController._advance_and_damage` 改格索引**。`in_blast` 是 Chebyshev ≤ 1 ＝ 剛好九格，
   所以「誰挨打」可以查表；舊寫法是 `O(敵人 × (節點 + 導管 × 每條的格數))`，而且每一對
   （敵人, 導管）都重算一次 `immune_indices()`。219 節點時 17.2 ms，佔整個 tick 的六成。
3. **`BattleController._edges` 不再用 `SessionState.node_at()`**（線性掃描 × 每條導管兩次
   ＝ `O(導管 × 節點)`）。610 節點／1107 導管時光這一支就 25.8 ms，**比整個解算器還貴**。
4. **拓樸一個 tick 只建一次**（`FlowNetwork.prepare()`）。三張資源網的圖逐位元相同，
   只有供需不同；舊寫法把 CSR、孿生邊、生成森林都算了三遍。

> 表中 219／610 兩格是 B2.1f.1 的複量（`ponytail-review` 砍掉沒人讀的入邊 CSR
> 與一趟多餘的節點掃描之後）。**219 節點因此進到 8 ms 上限之內**（8.1 → 7.8，
> 三次複量 7.7／7.7／7.8）。

**還沒到的地方**：**219 節點在上限內、610 節點是 20.7 ms**，交叉點落在兩者之間。
剩下的成本三分之二在解算器本身（三張網 × 三次迭代 × `O(V+E)`）。
20.7 ms 仍只佔 `TICK = 100 ms` 的兩成，**功能上不是阻斷**，但 RG-8 的另一半——
**渲染 23 FPS**——這一批完全沒有碰，仍然掛著（見 §5.1）。
